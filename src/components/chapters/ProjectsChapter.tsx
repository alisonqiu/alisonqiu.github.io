  ...
  return (
    <>
      ...
      <div className="paper-texture relative flex h-full flex-col overflow-hidden p-4 sm:p-5">
        ...
        <div className="mb-3 flex flex-wrap gap-1" role="tablist" aria-label="Projects">
          {PROJECTS.map((project, i) => (
            <button
              ...
              className={`bookmark-tab text-[10px] font-medium text-white ${
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="font-mono text-[10px] text-ink-muted">{active.date}</p>

            <p className="mt-3 text-xs leading-relaxed text-ink-muted">{active.overview}</p>

            <div className="page-scroll mt-3 flex-1 space-y-3 overflow-y-auto pr-1">
              <div>
                <span className="font-mono text-[10px] uppercase tracking-wider text-gold">
                  Problem
                </span>
                <p className="mt-1 text-xs leading-relaxed text-ink-muted">{active.problem}</p>
              </div>
              <div>
                <span className="font-mono text-[10px] uppercase tracking-wider text-gold">
                  Solution
                </span>
                <p className="mt-1 text-xs leading-relaxed text-ink-muted">{active.solution}</p>
              </div>
            </div>

            <p
              className="handwriting mt-2 text-sm text-gold"
      ...
      <div className="paper-texture relative flex h-full flex-col overflow-hidden border-l border-ink/5 p-4 sm:p-5">
        <p className="mb-1 font-mono text-[10px] uppercase tracking-wider text-gold">
          Stack
        </p>
        ...
        <div className="flex flex-wrap gap-1.5">
          {active.technologies.map((tech, i) => (
            <motion.span
              key={tech}
              className="rounded border border-ink/10 bg-cream-dark/60 px-2 py-0.5 font-mono text-[10px] text-ink"
        ...
        {active.github && (
          <a
            ...
            className="mt-4 inline-flex items-center gap-1 font-mono text-[11px] text-gold hover:underline"
          >
            View on GitHub →
          </a>
        )}
        ...
        <div className="mt-auto">
          ...
          <svg viewBox="0 0 200 80" className="w-full opacity-25" aria-hidden="true">
            ...
            <text x="35" y="44" textAnchor="middle" fontSize="6" fill="#4a3f35" fontFamily="monospace">
              input
            </text>
            <text x="100" y="44" textAnchor="middle" fontSize="6" fill="#4a3f35" fontFamily="monospace">
              build
            </text>
            <text x="165" y="44" textAnchor="middle" fontSize="6" fill="#4a3f35" fontFamily="monospace">
              ship
            </text>
          </svg>
        </div>
